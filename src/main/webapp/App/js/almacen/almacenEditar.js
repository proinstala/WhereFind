...
function validarFormulario(idForm) {
    $(idForm).validate({
        ...
        submitHandler: function () {
            mostrarMensajeOpcion("Modificar Almacen", '¿Quieres realmente modificar los datos?')
                    .then((result) => {
                            ...
                            solicitudPut(`api/almacen/update/${almacenId}`, idForm, true)
                                    .then(response => {
                                        if (response.isError === 1) {
                                            ...
                                            mostrarMensaje("Modificado almacen.", `Se han modificado correctamente los datos del almacen ${response.data.id}`, "success");
                                            oldAlmacen = response.data;
                                            onDetectarCambiosModificarAlmacen(false);
                                            detectarCambiosFormulario(idFormAlmacen, onDetectarCambiosModificarAlmacen);
                                        }
